...
// src/App.jsx
// Updated with dual backend sync support (CouchDB / Cosmos DB)
...
// ✨ Updated sync imports with dual backend support
import { 
  getSyncStatus, 
  startSync, 
  stopSync, 
...
// Import version
const APP_VERSION = "2.1.0"; // Updated version with dual backend support
...
function AppContent() {
  const { isAuthenticated, isLoading, user, hasAnyRole } = useAuth();
  const [syncStatus, setSyncStatus] = useState("idle");
  const [backendType, setBackendType] = useState(getBackendType());
  const [networkStatus, setNetworkStatus] = useState(navigator.onLine);
  const [showSystemLogs, setShowSystemLogs] = useState(false);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (navigator.onLine) {
        startSync();
        systemLogger.logInfo(`Sync started with ${getBackendDisplayName()} backend`);
      }
    } catch (error) {
      console.error("[App] Sync initialization error:", error);
    }
      ...
      <div style={styles.container}>
        ...
        <header style={styles.header}>
          ...
          <div style={styles.headerRight}>
            ...
            {/* ✨ UPDATED: Sync Status with Backend Type */}
            <div style={styles.syncBadge}>
              ...
              <span style={styles.backendLabel}>
                {backendType === 'cosmosdb' ? 'Cosmos' : 'Couch'}
              </span>
...
const styles = {
  ...
  // ✨ NEW: Sync badge with backend indicator
  syncBadge: {
    display: "flex",
    alignItems: "center",
    gap: "6px",
